...
// This is a temporary version without dependency injection for backward compatibility
func GeneratePDFResumeFromHTMLFile(c *gin.Context) {
	// Delegate to the new version with nil resumeService
	// This will work but won't save to history
	GeneratePDFResumeFromHTMLFileWithService(nil)(c)
}

// GeneratePDFResumeFromHTMLFileWithService is the improved version with dependency injection
func GeneratePDFResumeFromHTMLFileWithService(resumeService *services.ResumeService) gin.HandlerFunc {
	return func(c *gin.Context) {
		fmt.Println("GeneratePDFResumeFromHTMLFile handler called")

		// Try to get user email from request (could be form field or header)
		...
		// Save to resume history if we have a resumeService and user email
		if resumeService != nil && userEmail != "" {
			// Try to get user ID from email
			// Note: This would require a UserModel to look up the user
			// For now, we'll log that we couldn't save to history
			fmt.Printf("Resume generated for user %s but cannot save to history without user ID\n", userEmail)
		}
...
func DownloadResume(db *sql.DB) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if userExists && dbExists {
			...
			go func(savedKey string) {
				userIDInt, ok := userID.(int)
				if !ok {
					if v, okFloat := userID.(float64); okFloat {
						userIDInt = int(v)
					} else {
						fmt.Printf("unable to convert userID %v to int\n", userID)
						return
					}
				}
				...
				VALUES ($1, $2, $3, $4)
				`
				if _, insertErr := db.Exec(query, userIDInt, resumeName, savedKey, time.Now()); insertErr != nil {
					fmt.Printf("Error saving to resume history: %v\n", insertErr)
				} else {
					fmt.Printf("Successfully saved to resume history\n")
				}

				cleanupOldResumes(db, userIDInt)
			}(key)
		} else {
